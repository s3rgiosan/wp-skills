...
| Permanent finding IDs | plugin `SKILL.md` → Finding IDs are permanent | Area prefixes `G-`, `P-<slug>-`, `T-<slug>-`, mapping one to one onto each component audit's local IDs. Owner answers change severity in place with a severity note. |
...
| Privacy and data hygiene: where personal data lives, exporters and erasers in custom code, real production data in dev or staging (dumps described, never read) | `references/access-and-privacy.md` §4 | grep recipes |
...
- **Plain language.** No process words in the report (orchestrator, scanner, subagent, brief names, section files); evidence uses the plain labels in `references/report-template.md` → Evidence labels, with the legend in the Method appendix.
...
- **IDs:** `G-` general, `P-<slug>-` plugins, `T-<slug>-` themes (for example `P-acme-forms-H1`), mapping one to one onto each component audit's local IDs (never renumber when building the annexes). Owner answers change severity in place with a severity note; IDs are kept.
