...
| **Open registration and default role** | `users_can_register`, `default_role` (single site); `registration` site option (multisite) | Registration open with a default role above Subscriber: **High**. Open with Subscriber: note, and every subscriber-exploitable finding applies (plugin skill's subscriber-exploitable rule). |
