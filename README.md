...
Verification-first code audit for WordPress plugins. Six-phase workflow (discover → tool scan → manual read → verify → reproduce (optional) → report) covering security, performance, WordPress coding standards, WordPress.org Plugin Directory guidelines, and cross-plugin integration. Produces a dated `AUDIT-<yyyy-mm-dd>.md` (written to a location you choose, defaulting to a non-public one like `.claude/`) with severity-sorted findings (Critical / High / Medium / Low / Info), fix recommendations, a verified-false appendix, a collected list of `[DECISION]` questions only the plugin's owner can answer, and a final **GO / NO-GO / GO WITH FIXES** verdict. Works against a local plugin directory, a single file, or a remote source (wp.org slug, GitHub URL).
...
### [wp-theme-code-audit](./wp-theme-code-audit)

Theme counterpart of `wp-plugin-code-audit`: a verification-first audit for WordPress themes (block, classic, hybrid, child). Same six phases, severity rubric, `[DECISION]` findings and verdict rules, plus theme-specific checks: kses and capability filters widened below `unfiltered_html`, REST meta without a real `auth_callback`, contributor publishing bypasses, render-by-ID, block render files and Interactivity API context, patterns, Global Styles CSS, DOM XSS, third-party and bundled scripts, theme review standards, front-end performance, and child-theme overrides diffed against the parent. Produces a dated `THEME-AUDIT-<yyyy-mm-dd>.md` with a **GO / NO-GO / GO WITH FIXES** verdict. Requires `wp-plugin-code-audit`.

**Triggers on:** "audit this theme", "review the theme", "is this theme secure", "theme security review", "is this theme safe to install", "review this block theme", "review this child theme".
...
Translate a v0 (Vercel) design — React/Next + Tailwind + shadcn/ui — into a WordPress block theme (FSE). No automatic converter exists, so the skill splits the work into the mechanical parts it automates (per-breakpoint Playwright capture of the rendered design, mapping Tailwind v3/v4 tokens to `theme.json`, and a design-vs-rebuild parity gate) and the judgment parts it guides: rebuilding each route and section as templates, patterns, content, and Interactivity API blocks. Covers current block-theme practice (theme.json v3, style variations for dark mode, block bindings, root-padding-aware alignments) and the Interactivity API (server-seeded state, `data-wp-*` directives). Ships three scripts (`capture.mjs`, `tokens.mjs`, `compare.mjs`) and composes with `wp-block-themes` / `wp-patterns` / `wp-block-development` / `wp-interactivity-api`.
