...
| Bundled libraries in **every** component (custom, committed and managed): jQuery and jQuery UI, sliders, lightboxes, document viewers, Select2, Moment, Lodash, Handlebars, TinyMCE copies and others, checked against each library's own advisories | `references/bundled-libraries.md` | `scripts/bundled-libs.sh --osv` |
...
| **Untracked local artifact** | A dump, archive, log, export, backup or IDE/OS file that exists only on the local copy (not tracked in git or its history, not on production, not copied by a non-clean-checkout deploy) is out of scope: not a finding, not verified false, not mentioned anywhere in the report. Tables that exist only in a local database are the same: at most an open question about production. Rule: `references/inventory.md` §11. |
