...
2. **Tokens → theme.json.** `scripts/tokens.mjs <config-or-css>` → a `theme.json` `settings` fragment. Refine against `references/tokens-mapping.md` (fluid type, nested colors, edge cases). Tokens **before** patterns. Delegate to the **wp-block-themes** skill.
3. **Scaffold.** `style.css`, `theme.json` (merge step 2), `templates/`, `parts/`, `patterns/`, `styles/`, `functions.php`, `package.json` (`@wordpress/scripts`), `src/`. Delegate to the **wp-block-themes** skill.
4. **Layout.** Header/footer → `parts/*.html`. Page types → `templates/*.html` (index, front-page, page, single, archive, 404). Delegate to the **wp-block-themes** skill.
...
6. **Interactivity.** Accordions, tabs, sliders, mobile nav → **Interactivity API** (`data-wp-*` + store), server-rendered then hydrated; cross-region controls share one namespace. See `references/interactivity-recipes.md`. Delegate to the **wp-interactivity-api** skill.
